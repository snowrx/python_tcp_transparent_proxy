...
class TestContinuousCircularBuffer(unittest.TestCase):
    def test_basic_read_write(self):
        ...
        write_view = buf.get_writable_view()
        data = b"hello"
        write_view[: len(data)] = data
        buf.advance_write(len(data))

        read_view = buf.get_readable_view()
    ...
    def test_simultaneous_view_operations(self):
        ...
        self.assertEqual(len(buf.get_readable_view()), 0)

        # 4. Advance write. Now it should be readable.
        buf.advance_write(len(data))
        new_read_view = buf.get_readable_view()
        ...
        write_view_2 = buf.get_writable_view()  # length 5, starts at index 5
        self.assertEqual(len(write_view_2), 5)

        # Get readable view for existing data
        read_view_2 = buf.get_readable_view()  # length 5, starts at index 0
        ...
        self.assertEqual(buf.get_readable_view().tobytes(), b"ABCDEFG")
    ...
    def test_held_views_sequential_ops(self):
        ...
        buf = ContinuousCircularBuffer(10)
        # Setup initial data
        w = buf.get_writable_view()
        w[0:3] = b"XYZ"
        buf.advance_write(3)

        # --- Iteration 1 ---
        # 1. Get both views
        rv1 = buf.get_readable_view()  # "XYZ"
        wv1 = buf.get_writable_view()  # next 7 bytes
        ...
        self.assertEqual(buf.get_readable_view().tobytes(), b"12")

        # --- Iteration 2 ---
        # 1. Get both views again
        rv2 = buf.get_readable_view()  # "12"
        wv2 = buf.get_writable_view()  # next 8 bytes
        ...
        self.assertEqual(buf.get_readable_view().tobytes(), b"345")
